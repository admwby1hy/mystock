...
def set_ndays_average_pb_and_pe(df,ndays):
    ...
    for i in range( len(df) - ndays ):
        ...
        trade_date = df.loc[j]['trade_date']
        pe = df.loc[j]['pe_ttm']
        pb = df.loc[j]['pb']
...
def moni(dif_pb_dict,stock_df,ndays):
    ...
    for i in range(len(stock_df)):
        ...
        if i%22 == 0:
            ...
            if dif_pb < 0:  
                ...
                rate_list = rate_list.append([trade_date[0:4],trade_date[0:6],-thismoney])
    ...
    rate_list = rate_list.append([trade_date[0:4],trade_date[0:6],totalvolume])
    rate_df = pd.DataFrame(rate_list, index=['操作年份','操作月份','发生金额'])

    irr_rate =np.irr(rate_df['发生金额'])
